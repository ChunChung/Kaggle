...
#setwd(mainDir)
			path(mainDir,pathID, driverID,submitPath)  
	}

path <- function(mainDir,pathID,driverID,submitPath){
	readPath=paste(mainDir,pathID,sep='/')
		data=read.csv(readPath)
#Calculate the parameters
		distance=sqrt(diff(data$x)^2 + diff(data$y)^2)
...
#Output
		...
		final=paste(dirr,pathID,sep="/")
		output = data.frame(distance=distance,speed=speed,acceleration=acceleration,radius=radius)
		write.csv(output, final, row.names=FALSE)
}
...
submitPath="/Users/chunchung/Documents/Pitt/2015 spring/Data Mining/Project/data"

#for(driverID in 1039){
#  for(pathID in 1:200){
#    driver(dir,submitPath,driverID,pathID)
#  }
#}

train <- getTrainingData(submitPath, 2409)
